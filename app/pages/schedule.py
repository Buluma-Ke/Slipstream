...
def layout():
    return html.Div([
        # Hidden local storage to govern active year filter state
        dcc.Store(id='schedule-active-year-store', data=2025),
        ...
        html.Div([
            ...
            html.Div([
                html.Div([
                    DashIconify(icon='tabler:flag', width=13, style={'marginRight': '5px', 'color': '#E8002D'}),
                    html.Span('Season', className='pill-label', style={'marginBottom': '0', 'marginRight': '6px'}),
                    html.Span(id='schedule-pill-year-display', children='2025'),
                ], className='year-pill-single', id='schedule-year-pill-toggle'),

                # Interactive Dropdown items menu
                html.Div(
                    [html.Div(str(y), id={'type': 'schedule-year-pill', 'index': y}, className='year-dropdown-item')
                     for y in range(2025, 2017, -1)],
                    id='schedule-year-pill-dropdown',
                    className='year-pill-menu',
                    style={'display': 'none'},
                ),
                html.Div(id='schedule-year-overlay', className='year-pill-overlay', style={'display': 'none'}, n_clicks=0),
            ], style={'position': 'relative', 'display': 'flex', 'alignItems': 'center', 'gap': '8px'}),
        ], className='home-top-row'),

        # Race cards rendering grid container
        html.Div(id='schedule-cards', className='schedule-grid'),
    ], className='home-wrapper')
